...
@export var vehicle : VehicleCarBody3D
...
func _process(delta):
	...
	var center_of_gravity := vehicle.vehicle_body_3d.global_transform * vehicle.vehicle_body_3d.center_of_mass
	...
	if debug_sets[current_debug_set] == "Inputs" or debug_sets[current_debug_set] == "All":
		...
		debug_ui.draw_debug_line("throttle", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * vehicle.vehicle_body_3d.engine_force, Color.GREEN, false, Vector2(0.0, 0.0))
		...
		debug_ui.draw_debug_line("torque", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.vehicle_body_3d.engine_force / vehicle.MAX_FORCE), Color.AQUA, false, Vector2(2.0, 0.0))
		...
		debug_ui.draw_debug_line("brake", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.vehicle_body_3d.brake), Color.RED, false, Vector2(6.0, 0.0))
	...
	for wheel in vehicle.wheel_array:
		...
		if debug_sets[current_debug_set] == "Suspension Forces" or debug_sets[current_debug_set] == "All":
			...
			debug_ui.draw_debug_line(wheel.name + "spring_length", wheel.global_position, -wheel.global_transform.basis.y * (wheel.get_contact_point() / (wheel.wheel_radius+wheel.wheel_rest_length)), Color.GREEN, false, Vector2(4 * signf(wheel.position.x), 0))
			
		
		if debug_sets[current_debug_set] == "Tire Forces" or debug_sets[current_debug_set] == "All":
			var wheel_force : Vector3 = ((wheel.global_transform.basis.x * wheel.get_contact_normal().x) + (wheel.global_transform.basis.z * wheel.get_contact_normal().y)) * 0.0002
			var wheel_slip : Vector3 = ((wheel.global_transform.basis.x * wheel.get_skidinfo()) + (wheel.global_transform.basis.z * wheel.get_skidinfo())) * 2.0
			debug_ui.draw_debug_line(wheel.name + "force", wheel.get_contact_point(), wheel_force, Color.RED)
			debug_ui.draw_debug_line(wheel.name + "slip", wheel.get_contact_point(), wheel_slip, Color.YELLOW)
